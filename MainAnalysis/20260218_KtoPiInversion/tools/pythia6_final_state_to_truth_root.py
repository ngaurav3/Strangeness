...
def main() -> int:
    if len(sys.argv) != 3:
        print(
            "Usage: pythia6_final_state_to_truth_root.py input_final_state.dat output.root",
            file=sys.stderr,
        )
        return 1
    ...
    tree = ROOT.TTree("Events", "Converted PYTHIA6 final-state hadrons")
    ...
    def flush_event():
        ...
        if event_count == 0 and pdg.size() == 0 and n_ch_eta05[0] == 0:
            return
        k_pi_pt0405[0] = (
    ...
    with open(input_path, encoding="ascii") as f:
        for raw_line in f:
            ...
            pzv = float(parts[6])
            mv = float(parts[7])
            weak_daughter = int(parts[8]) if len(parts) >= 9 else 0
            ...
            pdg.push_back(pid)
            is_weak_decay_daughter.push_back(weak_daughter)
            ...
            if is_counted_pion_for_ratio(pid, pxv, pyv, pzv):
                ...
                if weak_daughter == 0:
                    n_k_pt0405[0] += 1
            elif is_counted_proton_for_ratio(pid, pxv, pyv, pzv) and weak_daughter == 0:
                n_p_pt0405[0] += 1
