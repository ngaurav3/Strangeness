...
int PlotPhiSBHistograms(std::string input = "PhiSBHistograms.root",
  ...
  TH1D* h1 = nullptr;
  TH1D* h2 = nullptr;
  file.GetObject("hPhiSBMassAccepted", hAccepted);
  file.GetObject("hPhiSBMass1Tag", h1);
  file.GetObject("hPhiSBMass2Tag", h2);
  if (hAccepted == nullptr || h1 == nullptr || h2 == nullptr) {
    std::cerr << "Missing histograms in " << input << std::endl;
    return 1;
  }

  TH1D* hAcceptedDraw = static_cast<TH1D*>(hAccepted->Clone("hPhiSBAcceptedDraw"));
  TH1D* h1Draw = static_cast<TH1D*>(h1->Clone("hPhiSB1Draw"));
  TH1D* h2Draw = static_cast<TH1D*>(h2->Clone("hPhiSB2Draw"));
  ...
  TLegend leg1(0.68, 0.79, 0.88, 0.88);
  ...
  p2.cd();
  TH1D* h1Linear = static_cast<TH1D*>(h1->Clone("hPhiSB1Linear"));
  TH1D* h2Linear = static_cast<TH1D*>(h2->Clone("hPhiSB2Linear"));
  SetStyle(h1Linear, kBlue + 1);
  SetStyle(h2Linear, kRed + 1);
  h1Linear->SetTitle("");
  h1Linear->GetXaxis()->SetTitle("m(K^{+}K^{-}) [GeV]");
  ...
  c.SaveAs((outputDir + "/phi_sb_counts.pdf").c_str());
