...
FitSummary runFit(TH1D* hSB, const std::string& category, const std::string& model,
  ...
  c.SaveAs((outputDir + "/d0_sb_" + category + "_" + model + "_fit.pdf").c_str());
  delete hDisp;
...
int main(int argc, char* argv[]) {
  const std::string signalInput = getArgument(argc, argv, "--signal-input", "D0CombinedAssignmentHistograms.root");
  const std::string sbInput = getArgument(argc, argv, "--sb-input", "D0SBHistograms.root");
