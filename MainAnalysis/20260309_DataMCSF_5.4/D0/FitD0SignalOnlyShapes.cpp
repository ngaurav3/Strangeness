...
void fitCategory(TH1D* h, const std::string& category, const std::string& outputDir, std::ofstream& csv) {
  ...
  drawFitPage(h, models, results, best,
              category + " D^{0} signal-only MC fits",
              outputDir + "/d0_" + category + "_fit_scan.pdf");
...
int main(int argc, char* argv[]) {
  const std::string inputFileName = getArgument(argc, argv, "--input", "D0CombinedAssignmentHistograms.root");
