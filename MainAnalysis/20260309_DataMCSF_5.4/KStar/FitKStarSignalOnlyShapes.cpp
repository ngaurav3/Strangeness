...
TF1 buildModel(const std::string& model, const std::string& name, TH1D* h) {
  ...
  TF1 f;
  if (model == "TripleGaussian") {
    f = TF1(name.c_str(),
            "[0]*exp(-0.5*((x-[1])/[2])^2)+[3]*exp(-0.5*((x-[1])/[4])^2)+"
            "[5]*exp(-0.5*((x-[1])/[6])^2)",
            gFitMin, gFitMax);
    f.SetParameters(0.5 * maxY, 0.892, 0.015, 0.3 * maxY, 0.030, 0.2 * maxY, 0.060);
    f.SetParLimits(1, 0.84, 0.94);
    ...
    f = TF1(name.c_str(),
            ...
            gFitMin, gFitMax);
    f.SetParameters(0.40 * maxY, 0.892, 0.012, 0.28 * maxY, 0.025,
                    0.20 * maxY, 0.045, 0.12 * maxY, 0.080);
    f.SetParLimits(1, 0.84, 0.94);
    f.SetParLimits(2, 0.002, 0.08);
    f.SetParLimits(4, 0.002, 0.12);
    f.SetParLimits(6, 0.002, 0.18);
    ...
    f.SetParameters(maxY, 0.892, 0.020, 1.5, 5.0, 1.5, 5.0);
    f.SetParLimits(1, 0.84, 0.94);
    ...
    f.SetParameters(0.8 * maxY, 0.892, 0.020, 1.5, 5.0, 1.5, 5.0, 0.2 * maxY, 0.050);
    f.SetParLimits(1, 0.84, 0.94);
    f.SetParLimits(2, 0.002, 0.10);
    f.SetParLimits(3, 0.2, 8.0);
    f.SetParLimits(4, 1.2, 80.0);
...
void fitCategory(TH1D* h, const std::string& category, const std::string& outputDir, std::ofstream& csv) {
  ...
  drawFitPage(h, models, results, best,
              category + " K^{*} signal-only MC fits",
              outputDir + "/kstar_" + category + "_fit_scan.pdf");
...
int main(int argc, char* argv[]) {
  const std::string inputFileName = getArgument(argc, argv, "--input", "KStarSignalOnlyHistograms.root");
  ...
  TH1D* h1 = nullptr;
  TH1D* h2 = nullptr;
  inputFile.GetObject("hKStarMassKaonTag", h1);
  inputFile.GetObject("hKStarMassKaonPionTag", h2);
