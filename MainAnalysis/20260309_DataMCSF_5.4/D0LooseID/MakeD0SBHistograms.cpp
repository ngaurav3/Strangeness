...
int main(int argc, char* argv[]) {
  ...
  const std::string outputFileName =
      getArgument(argc, argv, "--output", "d0_looseid_sb_histograms.root");
  ...
  TH1D hMassKaonTag(
      "hD0SBMassKaonTag",
      "D^{0} same-event reco pairs, kaon-tag; m(K#pi) [GeV]; Assignments / bin",
      kMassBins, massMin, massMax);
  TH1D hMassKaonPionTag(
      "hD0SBMassKaonPionTag",
      "D^{0} same-event reco pairs, kaon+pion-tag; m(K#pi) [GeV]; Assignments / bin",
      kMassBins, massMin, massMax);
  TH1D hMassAccepted(
      "hD0SBMassAccepted",
      "D^{0} same-event reco OS pairs, accepted; m(K#pi) [GeV]; Assignments / bin",
      kMassBins, massMin, massMax);
  ...
  TNamed selection(
      "SelectionSummary",
      Form("Reco-only D0 same-event OS pairs: RecoGoodTrack==1, nonzero charge, "
           ...
           "The assumed kaon track must satisfy RecoPIDKaon>=1 for the kaon-tag histogram, and the "
