...
int main(int argc, char* argv[]) {
  const std::string nominalFileName =
      (argc > 1) ? argv[1] : "d0_looseid_signal_only_histograms.root";
  const std::string swappedFileName =
      (argc > 2) ? argv[2] : "d0_looseid_wrong_assignment_histograms.root";
  const std::string outputFileName =
      (argc > 3) ? argv[3] : "d0_looseid_combined_assignment_histograms.root";
  ...
  TNamed selection("SelectionSummary",
                   "Combined D0 LooseID signal-only histograms from nominal and swapped assignments");
  selection.Write();
  outputFile.Close();
